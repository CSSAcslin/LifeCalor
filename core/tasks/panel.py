...
from PyQt5.QtWidgets import (
    QDockWidget, QHBoxLayout, QProgressBar, QPushButton, QTreeWidget,
...
class TaskPanel(QDockWidget):
    """Compact multi-task view with independent progress and execution details."""
    ...
    def _add(self, task):
        ...
        cancel.clicked.connect(
            lambda _checked=False, task_id=task.task_id:
        ...
        self.tree.setItemWidget(item, 5, progress)
        self.tree.setItemWidget(item, 7, container)
        self._rows[task.task_id] = (item, progress, cancel)
        self._update(task)

    @staticmethod
    def _execution_text(task):
        if not task.actual_backend:
            return "等待规划"
        backend = task.actual_backend.upper()
        device = task.device if task.device and task.device != "CPU" else ""
        if device:
    ...
    def _update(self, task):
        ...
        item.setText(2, self._execution_text(task))
        item.setText(3, task.resource_summary or "-")
        item.setText(4, self.STATUS_TEXT.get(task.status, str(task.status)))
        if task.total > 0:
            ...
            progress.setRange(0, 0 if active else 1)
            if not active:
                progress.setValue(1)
        cancel.setEnabled(
            task.cancellable
        ...
        execution = []
        if task.stage:
            execution.append(f"阶段: {task.stage}")
        if task.requested_backend or task.actual_backend:
            execution.append(
                f"后端: {task.requested_backend or '-'} -> "
                f"{task.actual_backend or '-'}"
            )
        if task.device:
            execution.append(f"设备: {task.device}")
        if task.precision:
            execution.append(f"精度: {task.precision}")
        if task.resource_summary:
            execution.append(f"资源: {task.resource_summary}")
        if task.backend_reason:
            execution.append(f"原因: {task.backend_reason}")
        tooltip = f"任务 ID: {task.task_id}\n{detail}"
        if execution:
            tooltip += "\n" + "\n".join(execution)
        for column in range(8):
            item.setToolTip(column, tooltip)
        item.setToolTip(2, "\n".join(execution) if execution else tooltip)
        item.setToolTip(3, task.resource_summary or tooltip)
    ...
    def _set_elapsed(self, task):
        ...
        item.setText(
            6,
            f"{elapsed:.1f} s" if elapsed < 60 else f"{elapsed / 60:.1f} min",
        )
